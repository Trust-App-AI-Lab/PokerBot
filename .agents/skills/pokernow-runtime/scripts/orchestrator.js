...
class BotInstance {
  ...
  async _executeAction(action) {
    ...
    if (act === 'sit_out' || act === 'leave_seat' || act === 'stand_up') {
      const sent = this.client.emitEvent('action', { type: 'QNH' });
      log.info(`[${this.botName}] Seat: ${act} → ${sent ? 'OK' : 'FAIL'}`);
      return;
    }
    if (act === 'sit_back') {
      const sent = this.client.sitBack();
      log.info(`[${this.botName}] Seat: sit_back → ${sent ? 'OK' : 'FAIL'}`);
      return;
    }
    if (act === 'sit' || act === 'request_seat') {
      const seat = action.seat || this.seatIndex || 2;
      const stack = action.stack || this.stack;
      await this.client.requestSeat(seat, stack);
      log.info(`[${this.botName}] Seat: requested ${seat} stack=${stack}`);
      return;
    }
