 ...
 *
 * Keeps WebSocket alive, detects turns, communicates with Codex via files.
 *
 * How it works:
 *   1. Connects to Poker Now, stays connected (pings keep it alive)
 *   2. When it's bot's turn  → writes turn.json  (game state for agent to read)
 *   3. Agent writes action.json → bridge reads it, executes, deletes the file
 ...
 *   turn.json      — written when it's our turn (agent reads this)
 *   action.json    — agent writes here to send an action (bridge reads + deletes)
...
// ── Recent hand history for agent context ───────
...
`);
  ...
  gameState.on('state_updated', (hand) => {
    ...
    // Save current state (always available for agent to read)
  ...
  gameState.on('my_turn', (turnInfo) => {
    ...
    // Attach recent hand history for agent's context
    ctx.recentHands = getRecentHandsSummary(10);

    // Write turn.json for agent to read
  ...
  async function executeAction(action) {
    ...
    if (act === 'sit_out' || act === 'stand_up' || act === 'leave_seat') {
      // QNH queues sitting out / leaving the next hand while preserving the table flow.
      const sent = client.emitEvent('action', { type: 'QNH' });
      log.info(`>>> ${act} (QNH) → ${sent ? 'OK' : 'FAILED'}`);
      logHistory('action', { action: act, sent });
      return;
    }
    if (act === 'sit_back') {
  ...
  3. agent reads turn.json, decides, writes action.json
