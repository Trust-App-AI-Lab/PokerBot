 ...
 *
 * Files (all in game-data/CoachBot/):
...
`);
  ...
  gameState.on('state_updated', (hand) => {
    const ctx = gameState.getAgentContext();
  ...
  gameState.on('my_turn', (turnInfo) => {
    ...
    const ctx = gameState.getAgentContext();
    ctx.isMyTurn = true;
    // Ensure turn-specific fields are set from turnInfo
    ctx.callAmount = turnInfo.callAmount;
    ctx.minRaise = turnInfo.minRaise;
    ctx.maxRaise = turnInfo.maxRaise;
  ...
  async function executeAction(action) {
    ...
    // Seat actions
    if (act === 'sit_out' || act === 'stand_up' || act === 'leave_seat') {
      const sent = client.emitEvent('action', { type: 'QNH' });
      log.info(`Action: ${act} → ${sent ? 'OK' : 'FAIL'}`);
      return;
    }
    if (act === 'sit_back') {
  ...
  const TABLE_HTML_PATH = path.join(__dirname, '..', '..', '..', '.agents', 'skills', 'poker-server', 'public', 'poker-table.html');
  ...
  const httpServer = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && req.url === '/state') {
      ...
      if (state) {
        // Mark isMyTurn from turn file (fields already in state from getAgentContext)
        const turn = readJSON(TURN_FILE);
        if (turn) {
          state.isMyTurn = true;
          if (turn.callAmount != null) state.callAmount = turn.callAmount;
          if (turn.minRaise != null) state.minRaise = turn.minRaise;
          if (turn.maxRaise != null) state.maxRaise = turn.maxRaise;
        }
    ...
    // GET /history — compatible with poker-client.js API
    // ?sessions → list session files, ?last=N → last N hands, (no params) → all hands
    if (req.method === 'GET' && (req.url === '/history' || req.url.startsWith('/history?'))) {
      try {
        const params = new URL(req.url, `http://localhost:${httpPort}`).searchParams;

        // List sessions — coach-ws uses a single history file, return as array
        if (params.has('sessions')) {
          const files = fs.existsSync(HISTORY_FILE) ? [path.basename(HISTORY_FILE)] : [];
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify(files));
          return;
        }

        // Parse events from history file
        let events = [];
        ...
        // Reconstruct hands (same logic as poker-client.js)
        const hands = [];
        let cur = null;
    ...
    // GET /config → return defaults (pokernow games are configured on pokernow.com, not here)
    ...
    if (req.method === 'POST' && req.url === '/inject-state') {
      ...
      req.on('end', () => {
        try {
          ...
          // Broadcast to browser WebSocket clients via forward-declared wsBroadcast
          if (typeof wsBroadcast === 'function') {
            wsBroadcast('state', { state });
            if (state.myCards) wsBroadcast('cards', { cards: state.myCards });
          }
          res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  wss.on('connection', (clientWs) => {
    ...
    // Send welcome (autoJoinName tells browser to skip join screen and auto-join)
    ...
    clientWs.on('message', (raw) => {
      try {
        ...
        } else if (['sit_out', 'sit_back', 'pause', 'resume', 'start'].includes(msg.type)) {
          const action = {
            action: msg.type === 'start' ? 'next_hand' : msg.type,
          };
          executeAction(action).catch(e => {
            log.warn(`WS control failed: ${e.message}`);
            wsSend(clientWs, 'error', { message: e.message });
          });
        } else if (['rebuy', 'settings', 'kick'].includes(msg.type)) {
          wsSend(clientWs, 'error', {
            message: `${msg.type} is not available in PokerNow adapter mode`,
          });
        }
      } catch {}
    });
  ...
  buildPokerServerState = function(hand) {
    ...
    const players = (hand.players || []).map((p, i) => ({
      ...
      bet: p.bet || 0,
      folded: !!p.folded,
      sittingOut: !!p.sittingOut || p.status === 'sittingOut',
    ...
    // Build positions map
    const positions = {};
    const readyPlayers = (hand.players || []).filter(p => !p.folded && !p.sittingOut && (p.status === 'inGame' || !p.status));
    const n = readyPlayers.length;
