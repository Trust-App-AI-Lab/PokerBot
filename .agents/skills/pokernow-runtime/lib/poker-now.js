...
class PokerNowClient extends EventEmitter {
   ...
   * @param {string} [opts.workDir]    - Base directory (default: project root, i.e. pokernow-runtime/)
   * @param {string} [opts.profileDir] - Bot profile directory; overrides auto-detection.
   *                                     When omitted, defaults to `{workDir}/bot-management/bots/{botName}/`
  ...
  constructor({ gameUrl, botName = 'ARIA_Bot', logger, workDir, profileDir, cookieFile }) {
    ...
    // Bot profile directory: explicit > auto (PokerBot/.agents/skills/bot-management/bots/{botName}/)
    this.profileDir = profileDir || path.join(this.workDir, '.agents', 'skills', 'bot-management', 'bots', botName);
  ...
  //   - 42["rup", {...}]                   — round update
  //
  // Actions sent as: 42["action", {type:"PLAYER_FOLD"}]
