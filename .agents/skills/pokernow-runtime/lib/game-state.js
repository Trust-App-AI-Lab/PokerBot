...
class GameState extends EventEmitter {
  ...
  _parsePlayers(playersObj, seats, frame = {}) {
    ...
    // Seated statuses — players visible at the table.
    // PokerNow status field tracks table presence (not hand state):
    //   "inGame"  = seated, active player
    //   "sittingOut" = seated, temporarily inactive
    //   ""        = left seat / not at table
    //   "requestedGameIngress" / "waitingToJoin" / "waiting" = queued, not seated
    //   "leftGame" / "watching" = inactive / spectator
    const SEATED_STATUSES = new Set(['inGame', 'sittingOut']);

    for (const [pid, p] of Object.entries(playersObj)) {
      const playerStatus = p.status || '';

      // Skip players not actively seated at the table
      // (always include ourselves even if status is stale)
      const isMe = pid === this.playerId || (this.botName && p.name === this.botName);
      if (!isMe && !SEATED_STATUSES.has(playerStatus)) {
        continue;
      }
      ...
      const player = {
        ...
        gameStatus: gs,
        folded: gs === 'fold',
        sittingOut: playerStatus === 'sittingOut',
        cards:  [],
  ...
  _parseEventString(str) {
    ...
    // Parse win results: "PlayerX wins 400", "PlayerX wins 400 with Two Pair", "PlayerX collected 400 from pot"
    const winMatch = str.match(/(.+?)\s+(?:wins?|collected|gains?)\s+(\d+)(?:\s+with\s+(.+?))?(?:\s+from|\s*$)/i);
    if (winMatch) {
      const result = { winner: winMatch[1].trim(), amount: Number(winMatch[2]), hand: winMatch[3] || null, cards: null };
  ...
  getAgentContext() {
    ...
    // Find current actor (whose turn it is)
    const currentActor = h.isMyTurn ? h.myName
      : (h.players.find(p => p.gameStatus === 'action' || p.gameStatus === 'yourTurn') || {}).name || null;

    // Build positions map (approximate from seat order + dealer)
    const positions = {};
    const seated = h.players.filter(p => p.seat != null && !p.sittingOut);
    if (seated.length > 0 && h.dealer != null) {
      // Simple position assignment for compatibility
      const dealerIdx = seated.findIndex(p => p.seat === h.dealer);
      if (dealerIdx >= 0) {
        const posNames = ['BTN', 'SB', 'BB', 'UTG', 'HJ', 'CO'];
        seated.forEach((p, i) => {
          const offset = (i - dealerIdx + seated.length) % seated.length;
          if (offset < posNames.length) positions[p.name] = posNames[offset];
        });
      }
    }

    return {
      // Aligned with poker-server getPlayerState() format
      phase:          h.phase,
      paused:         false,
      handNumber:     h.handNumber || 0,
      pot:            h.pot,
      ...
      timestamp:      Date.now(),
      // Player-specific fields (same as poker-server getPlayerState)
      myCards:        h.myCards,
      myStack:        h.myStack,
      isMyTurn:       false,  // overridden by /state handler when applicable
