...
import sys
import numpy as np
from range_parser import parse_range, RANGE_PRESETS, resolve_preset_range
...
if __name__ == "__main__":
    ...
    villain_label = range_str
    preset = resolve_preset_range(range_str)
    if preset:
        preset_key, preset_range = preset
        if preset_key == range_str:
            print("Using preset: %s = %s" % (range_str, preset_range))
            villain_label = range_str
        else:
            print("Using preset: %s ~= %s = %s" % (range_str, preset_key, preset_range))
            villain_label = "%s (~%s preset)" % (range_str, preset_key)
    ...
    print("Villain: %s (%d combos)" % (villain_label, len(villain_range)))
    if board:
