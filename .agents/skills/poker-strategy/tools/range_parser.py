...
def parse_range(range_str: str, dead_cards: list[str] = None) -> list[tuple[str, str]]:
    ...
    """
    preset = resolve_preset_range(range_str)
    if preset:
        _, range_str = preset
...
def resolve_preset_range(pct: str) -> tuple[str, str] | None:
    """Resolve an exact or approximate percentage preset to (preset_key, range)."""
    key = pct.strip().lower()
    if key in RANGE_PRESETS:
        return key, RANGE_PRESETS[key]
    if not key.endswith("%"):
        return None
    try:
        value = float(key[:-1])
    ...
    numeric = []
    for preset_key in RANGE_PRESETS:
        if not preset_key.endswith("%"):
            continue
        try:
            numeric.append((float(preset_key[:-1]), preset_key))
        except ValueError:
            pass
    if not numeric:
        return None
    _, nearest_key = min(numeric, key=lambda item: (abs(item[0] - value), item[0]))
    return nearest_key, RANGE_PRESETS[nearest_key]


def get_preset_range(pct: str) -> str:
    """Get a preset range string by percentage, approximating to the nearest preset if needed."""
    resolved = resolve_preset_range(pct)
    if resolved:
        return resolved[1]
...
if __name__ == "__main__":
    ...
    preset = resolve_preset_range(range_str)
    if preset:
        preset_key, preset_range = preset
        if preset_key == range_str:
            print(f"Preset {range_str} = {preset_range}")
        else:
            print(f"Preset {range_str} ~= {preset_key} = {preset_range}")
        range_str = preset_range
