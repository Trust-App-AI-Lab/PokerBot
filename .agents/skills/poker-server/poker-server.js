...
// Event types: hand.started, player.action, street.dealt, hand.ended
const HISTORY_FILE = path.join(__dirname, 'history.jsonl');
let currentHandNumber = 0;

function logEvent(event) {
  appendJsonl(HISTORY_FILE, event, e => log.warn(`Failed to write history: ${e.message}`));
}

engine.on('hand_start', (data) => {
  log.info(`=== HAND #${data.handNumber} === Dealer: ${data.dealer}`);
  currentHandNumber = data.handNumber;
  // Build players map: { name: [cards, startingStack] }
  // Cards are REDACTED in history file — only revealed at hand_end for shown players.
  // This prevents information leaks if someone reads history.jsonl.
  const players = {};
  for (const name of engine.seatOrder) {
    const p = engine.players.get(name);
    if (p) players[name] = [[], p.stack + p.bet]; // cards hidden at start
  }
...
engine.on('hand_end', (data) => {
  ...
  // Cards are revealed HERE (not at hand_start) for information safety.
  const shown = [];
  const shownCards = {}; // { name: [card1, card2] } — only for players shown at showdown
  if (data.players) {
  ...
  // Auto-rebuy: busted players get auto-rebuyed (skip sitting-out players)
  for (const name of engine.seatOrder) {
    const p = engine.players.get(name);
...
engine.on('player_sit_back', (data) => {
  const seat = data.seat !== undefined ? ` at seat ${data.seat + 1}` : '';
  log.info(`🔙 ${data.name} sitting back in${seat}`);
...
const server = http.createServer((req, res) => {
  ...
  // POST /config → update config (e.g. {"turnTimeout": 180000, "autoStart": false})
  if (req.url === '/config' || req.url.startsWith('/config?')) {
    if (req.method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ turnTimeout: TURN_TIMEOUT, stack: STACK, smallBlind: SB, bigBlind: BB, autoStart: engine.autoStart }));
      return;
    }
    if (req.method === 'POST') {
      let body = '';
      ...
      req.on('end', () => {
        try {
          ...
          const nextSB = cfg.smallBlind != null ? Number(cfg.smallBlind) : SB;
          const nextBB = cfg.bigBlind != null ? Number(cfg.bigBlind) : BB;
          const settingsChanged = cfg.smallBlind != null || cfg.bigBlind != null || cfg.autoStart != null;
          if (settingsChanged) {
            const r = engine.updateSettings({
              smallBlind: nextSB,
              bigBlind: nextBB,
              autoStart: cfg.autoStart != null ? cfg.autoStart : engine.autoStart,
            });
            if (!r.ok) {
            ...
            SB = engine.smallBlind;
            BB = engine.bigBlind;
          }
          log.info(`⚙ Config updated: timeout=${TURN_TIMEOUT}ms blinds=${SB}/${BB} autoStart=${engine.autoStart} stack=${STACK}`);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ ok: true, turnTimeout: TURN_TIMEOUT, stack: STACK, smallBlind: SB, bigBlind: BB, autoStart: engine.autoStart }));
        } catch (e) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
  ...
  if (req.method === 'GET' && (req.url === '/history' || req.url.startsWith('/history?'))) {
    try {
      ...
      // Reconstruct hands from event stream. Supports both the new structured
      // event names and legacy hand_start/action/board/hand_end JSONL.
      ...
      if (playerName) {
        result = result.map(h => {
          // Skip incomplete hands with no shown data
          if (h.incomplete) return h;
      ...
      if (rawMode) {
        ...
        if (lastN > 0) {
          ...
          rawEvents = eventsForHands(events, result);
        }
        rawEvents = filterRawEventsForPlayer(rawEvents, playerName);
        res.writeHead(200, { 'Content-Type': 'application/json' });
...
wss.on('connection', (ws) => {
  clients.set(ws, { playerName: null });
