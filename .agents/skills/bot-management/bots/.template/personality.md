...
| `equity`    | % win vs a villain range (optional board)          | `source ./paths.env 2>/dev/null || true; "${PY:-python}" .agents/skills/poker-strategy/tools/equity.py Ah Kh "QQ+" Td 7d 2c` | `Equity: 16.5% · Win: 16.5% · Tie: 0% · Lose: 83.5%` |
...
| `evaluator` | Final hand rank from 5-7 cards                     | `source ./paths.env 2>/dev/null || true; "${PY:-python}" .agents/skills/poker-strategy/tools/evaluator.py Ah Kh Qh Jh Th`    | `Royal Flush (class=9, tiebreak=(12,))` |
...
<!--
Writing guide:
- Frontmatter: only `model`. Use a Codex-compatible model such as `gpt-5.4`.
