...
- Strategy: reads `poker-agent/strategy/preflop.md` (RFI, facing raises, 3-bets), `poker-agent/strategy/postflop.md` (board texture basics, c-bet spots, when to give up), and `poker-agent/strategy/range.md` (understands range concept, knows preflop range widths by position, but doesn't do precise street-by-street narrowing).
