...
class GameState extends EventEmitter {
  ...
  _parsePlayers(playersObj, seats, frame = {}) {
    ...
    // Active statuses — players seated and participating at the table.
    ...
    //   "leftGame" / "sittingOut" / "watching" = inactive
    // Only include players with an active status in the player list.
    const ACTIVE_STATUSES = new Set(['inGame']);
    ...
    for (const [pid, p] of Object.entries(playersObj)) {
      ...
      if (!isMe && !ACTIVE_STATUSES.has(playerStatus)) {
        continue;
      }
