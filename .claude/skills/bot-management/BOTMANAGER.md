...
- `botmanager-init.md` (this directory) — Init prompt: load personality + strategy, confirm ready
- `botmanager-turn.md` (this directory) — Turn prompt: read state, decide, submit action, EXIT
...
```bash
# Start BotManager — just the polling loop, no init/join
bash .claude/skills/bot-management/botmanager.sh --server http://localhost:3457 &

# Stop BotManager
bash .claude/skills/bot-management/stop-botmanager.sh
...
Polls `pending-turns.json`, writes `action.json`. See `botmanager-init.md` + `botmanager-turn.md` (this directory).
