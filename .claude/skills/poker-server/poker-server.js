...
engine.on('hand_end', (data) => {
  ...
  logEvent({ type: 'hand_end', hand: currentHandNumber, results, shown, shownCards, stacks });
    ...
    if (req.method === 'POST') {
      ...
      req.on('end', () => {
        try {
          ...
          if (cfg.smallBlind != null) { SB = Number(cfg.smallBlind); }
          if (cfg.bigBlind != null) { BB = Number(cfg.bigBlind); }
          // Validate BB > SB before applying to engine
          if (SB > 0 && BB > 0 && BB > SB) {
            engine.updateSettings({ smallBlind: SB, bigBlind: BB });
          } else if (SB > 0 && BB > 0) {
            log.warn(`Invalid blinds: SB=$${SB} BB=$${BB} — BB must be greater than SB`);
          }
