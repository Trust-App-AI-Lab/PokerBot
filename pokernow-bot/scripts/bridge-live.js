 ...
 *
 * Files (all relative to pokernow-bot/):
...
// Bot profile directory — each bot's files live under PokerBot/bot_profiles/{botName}/
...
async function main() {
  if (!CONFIG.gameUrl) {
    log.error('No GAME_URL set. Put it in pokernow-bot/.env');
    process.exit(1);
  }
...
`);
  ...
  const client = new PokerNowClient({
    ...
    profileDir: PROFILE_DIR,    // bot_profiles/{botName}/
  });
